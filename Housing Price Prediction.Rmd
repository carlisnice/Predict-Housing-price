...
#Let's Graph the locations of the houses
```{r}
library(MASS)
library(sp)
library(rworldmap)
...
```
#Machine Learning methods(OLS,Ridge,Lasso,Elastic net)
```{r}
library(caret)
...
#These 4 models are all very similar
#Elastic net performs the best from the RMSE
#However, Lasso seems to do better job on Rsquared
summary(res)
#These boxplots also show that the 4 models are similar
bwplot(res)
```
